...
def process_data(data, start, end):
    sub_data = data[(data['time']>= start) & (data['time'] < end)]
    if len(sub_data) == 0:
        return 0, 0, []
    ...
    peak_times = sub_data.iloc[peaks]['time'].values if len(peaks) > 0 else []   #如果检测到峰值，就返回峰值时间点
    # print(peak_times)
...
def main(data):
    ...
    all_peak_times = data['time'].iloc[peaks].values  # 使用全局峰值索引获取峰值时间点
    
    print(velocities_smoothed)
